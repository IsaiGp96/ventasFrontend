...
import { fetchOrden } from "../../api/compras.js";
...
export default function EditarOrdenPage() {
    ...
    useEffect(() => {
        Promise.all([fetchOrden(id), fetchProveedores(), fetchProductos()])
            .then(async ([orden, p, pr]) => {
                ...
                setIdProveedor(String(orden.idProveedor ?? ""));
                setEsLote(Boolean(orden.esLote));
                setCostoLote(orden.costoLote ? String(orden.costoLote) : "");
                setPiezasLote(orden.piezasLote ? String(orden.piezasLote) : "");
                setFlete(orden.flete ? String(orden.flete) : "0");

                // Cargar productos filtrados por proveedor
                const filtrados = pr.filter(
                    prod => String(prod.idProveedor) === String(orden.idProveedor)
                );
                setProductosFiltrados(filtrados);

                // Cargar variantes de esos productos
                const vars = {};
                await Promise.all(filtrados.map(async prod => {
                    try {
                        const p = await fetchProducto(prod.id);
                        vars[String(prod.id)] = p.variantes ?? [];
                    } catch {}
                }));
                setVariantesPorProducto(vars);

                // Mapear detalles existentes — necesitamos idProducto de la variante
                const lineasCargadas = orden.detalles.map(d => {
                    // Buscar el producto al que pertenece la variante
                    const idProd = Object.entries(vars).find(([, variantes]) =>
                        variantes.some(v => String(v.id) === String(d.idVariante))
                    )?.[0] ?? "";

                    return {
                        idProducto:    idProd,
                        idVariante:    String(d.idVariante ?? ""),
                        sku:           d.sku ?? "",
                        cantidad:      String(d.cantidad ?? ""),
                        costoUnitario: String(d.costoUnitario ?? ""),
                    };
                });
                setLineas(lineasCargadas.length > 0
                    ? lineasCargadas
                    : [{ idProducto: "", idVariante: "", sku: "", cantidad: "", costoUnitario: "" }]
                );
            })
    ...
    async function handleProveedorChange(nuevoId) {
        ...
        setLineas([{ idProducto: "", idVariante: "", sku: "", cantidad: "", costoUnitario: "" }]);
    ...
    async function handleSubmit() {
        // Validar variantes únicas
        const idsVariantes = lineas.map(l => l.idVariante).filter(Boolean);
        const duplicados   = idsVariantes.filter((id, idx) => idsVariantes.indexOf(id) !== idx);
        if (duplicados.length > 0) {
            setError("Hay variantes repetidas. Cada variante solo puede aparecer una vez.");
            return;
        }
        const sinVariante = lineas.filter(l => !l.idVariante);
        if (sinVariante.length > 0) {
            setError("Todas las líneas deben tener una variante seleccionada.");
            return;
        }
        ...
        try {
            ...
            const res = await apiPut(`/api/compras/${id}`, payload);
            if (!res.ok) {
    ...
    return (
        <div className="p-6 lg:p-8 max-w-5xl space-y-6">
            ...
            <div className="flex items-center gap-3">
                <button onClick={() => navigate(`/compras/${id}`)}
                ...
                <div>
                    <h1 className="text-xl font-semibold text-neutral-900">
                        Editar orden #{String(id).padStart(4, "0")}
                    </h1>
                    <p className="text-sm text-neutral-500 mt-0.5">Solo disponible en borrador</p>
                </div>
            </div>
                        ...
                        <div className="flex gap-3 mb-4">
                            {[
                                ...
                                <button key={String(op.value)} type="button"
                                    onClick={() => { setEsLote(op.value); setLineas([{ idProducto: "", idVariante: "", sku: "", cantidad: "", costoUnitario: "" }]); }}
                ...
                <div className="bg-white border border-neutral-200 rounded-xl">
                    ...
                    {lineas.map((linea, idx) => {
                        const variantesDisponibles =
                            ...
                            .filter(v =>
                                String(v.id) === String(linea.idVariante) ||
                                !lineas.some((l, i) =>
                                    i !== idx && String(l.idVariante) === String(v.id)
                                )
                            );
            ...
            <div className="flex gap-3">
                <button type="button" onClick={() => navigate(`/compras/${id}`)}
